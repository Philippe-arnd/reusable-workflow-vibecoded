...
Semgrep base config is always `p/security-audit p/javascript p/react p/nodejs`. Add project-specific rules via `semgrep-extra-config` (e.g. `p/typescript .semgrep.yml`). Semgrep itself is CLI-installed via `pip install "semgrep==$SEMGREP_VERSION"` with the version pinned inline in the run step (marked with a `# renovate: datasource=pypi depName=semgrep` comment) rather than the archived `semgrep/semgrep-action` — a requirements file living in this repo would not be reachable, since the security-scan job's checkout pulls the *calling* repo, not this one. The run step also greps semgrep's own output for its "rule validation failed" banner and reports a warning in the PR comment when present — this guards against a rule (custom or registry) silently failing to parse and never running, while the job still reports success. Note: comparing "rules loaded" vs "rules run" is *not* a useful signal for this — broad multi-language packs like `p/security-audit` legitimately load far more rules than apply to any single-language repo (e.g. this repo's ~302 loaded / ~108 run split is normal, not a bug), so that gap is expected and unrelated to whether anything is actually broken.

Before uploading, the SARIF is filtered to drop any result carrying a `suppressions` entry (i.e. anything silenced with an inline `// nosemgrep: <rule-id>` comment). Semgrep's own CLI findings count/exit code already excludes these, but the SARIF file legitimately still contains them per spec — and GitHub's code-scanning ingestion doesn't honor SARIF's `suppressions` field, so an unfiltered upload turns every deliberately-suppressed finding into a permanently open Security tab alert (found on JournalVibecoded, 2026-08-25 — 4 alerts, all pre-existing `nosemgrep` suppressions with a documented reason in the comment).
